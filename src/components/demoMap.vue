<template>
  <div class="map_demo">
    <div class="map_left">
      <baidu-map class="map" ak="kptRwmAUC3crfb6FRvYSGgc2TV4suVaP" :center="center" :zoom="map_zoom" @ready="map_handler" id="container">
      </baidu-map>
    </div>
    <div class="mark_sel">
      <div class="sel_sty">
        <span>地点:</span>
        <el-select v-model="marks" @change="markChange" multiple collapse-tags style="flex: 1;" placeholder="请选择">
...
export default {
  ...
  data () {
    return {
      ...
      options: [
        {
          value: '121.480491,31.235943',
          label: '上海'
        },
        {
          value: '113.269282,23.134401',
          label: '广州'
        },
        {
  ...
  methods: {
    map_handler ({ BMap, map }) {
      ...
      map.setMapStyleV2({
        styleId: 'ebc4597baaeadb2e7fac1e7cf6dca62e'
      });
      map.setMinZoom(5)
      this.positionTest(BMap, map);
      this.subrouter(BMap, map);
      this.drawBoundary(BMap, map);
    },
    // 地址选择
    markChange (val) {
      let removes = this.options.filter(e => val.indexOf(e.value) === -1)
      this.map.getOverlays().forEach(f => {
        removes.forEach(e => {
          if (f.type === 'mark') {
            if (Number(e.value.split(',')[0]) == f.point.lng && Number(e.value.split(',')[1]) == f.point.lat) {
    ...
    // 标注
    markSelect (e) {
      let poiont = new this.BMap.Point(e.split(',')[0], e.split(',')[1]);
      let marker = new this.BMap.Marker(poiont);
      marker['type'] = 'mark'
      this.map.addOverlay(marker);
    },
    // 定位
    positionTest (BMap, map) {
    ...
    // 路线
    subrouter (BMap, map) {
      var p1 = new BMap.Point(120.61990712, 31.31798737);
      var p2 = new BMap.Point(116.508328, 39.919141);
      var driving = new BMap.DrivingRoute(map, { renderOptions: { map: map, autoViewport: true } });
      driving.search(p1, p2);
      // var walking = new BMap.WalkingRoute(map, { renderOptions: { map: map, autoViewport: true } });
      // walking.search('苏州', '北京');
    },
    maoInit () {
    },
    // 只显示中国
